...
**A bash rule does not grant shell redirection.** A clause that creates or truncates a file (`>`, `>>`, `>|`, `&>`, `&>>`, `<>`, or `>&` with a non-fd target) must clear a second bar on top of matching a bash pattern: every redirect target has to be a path the same caller could have written with the `Write` tool — covered by an `alwaysAllowPathPatterns` `Write:` rule, or inside the session's own worktree. So `read` alone can't do `echo pwned > ~/.ssh/authorized_keys`, and `core`'s `^cat ` can't do `cat payload > ~/.zshrc`. Targets that can't be resolved statically — `$VAR`, `$(…)`, backticks, `~`, globs, or any relative path — always deny; the checker sees the command text, not the expanded value, and not the cwd the clause will actually run in. Input redirection (`<`, `<<`, `<<<`, `<&`) and fd duplication (`2>&1`, `>&2`) are ungated: neither writes a file.
...
- **A blocker always lands in the action's journal.** `WorkEngine.onStepFailed` appends a `blocked` entry unless the session already journaled for that step, so the Library's "Self-reported lessons" is never empty for an action that keeps failing (which is the only signal `meta.improve-actions` reads). Pass `{ journal: false }` for failures the action didn't cause — a daemon bounce, a workspace that wouldn't provision.
...
- **A redirect in an action's Bash command needs a `Write:` path grant.** `splitShellCommand` judges a clause by its leading command, so a redirection used to ride along invisibly — every `runner: claude` action could `cat x > ~/.zshrc` off `core`'s `^cat `. `Allowlist.allows()` now gates each redirect target as if it were a `Write` (see `redirectsAllowed` in `src/permissions/allowlist.ts`). If an action legitimately needs `cmd > somewhere`, give it a literal absolute target and an `alwaysAllowPathPatterns` entry like `Write:^/tmp/`, or write inside the session worktree — `> $OUT` and `> out.log` deny on principle, because the daemon can't know what they resolve to. A whole-tool `Bash` grant is exempt: it already means "run anything".
