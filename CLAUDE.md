...
**`resolve` means the PR merged — the policy enforces it, it isn't just skill prose.** A controller holding a `writable` workspace owns its PR, so `validateNext` refuses its `resolve` while `step.pr.prUrl` is set and `prState` isn't `merged`. `resolve` and `fail` used to skip every guard, which let a controller near the round wall pick the exit that *reads* as done: the egress-v2 step resolved at round 78 of 80 with its PR open, approved, and one check from merge, and the unfinished work dropped out of the cockpit. Three carve-outs, each load-bearing — `workspace.kind` (a `readonly` reviewer like `code.orchestrate-review` settles on a verdict it could never merge), an absent `prUrl` (the step never opened one), and `boundAction === 'code.merge-pr'` (that round re-reads GitHub, so it knows about a merge the watcher hasn't swept — and since the watcher only syncs *live* steps, a just-merged step's recorded `prState` is frozen at `open` the moment it resolves; 3 of the 4 such steps on disk are merged PRs whose facts never caught up). Paired with this, **`gate` is now exempt from the round budget**: the budget bounds autonomous work and a gate spends none until a human acts, so a controller out of rounds can hand the step over instead of being cornered into `fail` as its only legal exit. Pinned by `orchestrated-policy.test.ts`.
