...
`readArgsAllowed` is why `core` needs no regex: `cat`/`jq` may open `"$OUTPOST_ENVELOPE"` (recognised by spelling in ONE place — the checker sees command text, never the expansion), a path inside the session's own worktree, or a path a `Read:` rule grants. The whole-tool `Read` exemption means it binds exactly the actions inheriting `core` and no read grant — today `read.linear-issue`, `write.linear-comment`, `write.linear-issue`, `write.run-github-workflow`, none of which read anything but their envelope. Global scope carries `Read`, so interactive sessions are unaffected. `walkOperands` is the shared operand finder behind the read and file-op bars: they ask different questions, but the *finding* (a `<(…)` that isn't a literal path, a `--flag=value` hiding one, an unparseable metacharacter) is where the subtle failures live, so it exists once. See `allowlist-read-scope.test.ts`.
