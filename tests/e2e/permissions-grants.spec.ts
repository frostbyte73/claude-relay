...
// The Grants block's inline edit is the honest half of the "sessions and skills only" promise:
// a grant here is a real, persisted rule, but re-keying on edit (the id encodes the value) has
// to actually happen or the row silently starts addressing a dead id. The subtitle is the
// single most load-bearing sentence on the page — pinning its presence here, not just reading
// the source, is what stops a later refactor from quietly dropping it into a tooltip.
...
test('the grants block states its scope up front', async ({ outpostPage }) => {
  await openPermissions(outpostPage);
  await expect(outpostPage.locator('.perm-grants-subtitle')).toContainText('sessions and skills only', { ignoreCase: true });
});
