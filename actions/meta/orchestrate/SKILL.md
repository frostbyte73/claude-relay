...
- **Customer incident ticket** — investigate inline, put the verdict in `findings` (the verdict block), and emit `write.linear-issue` (the bug filing) → `write.linear-comment` (back on the incident ticket) in one pass, packing `verdict.writeup` / `verdict.customer_summary` into the write steps' `inputs`. Each `write.*` step runs unattended and drafts its own exact payload via `mcp__outpost__submit_write_draft`, parking itself for the user's approval before it posts — so you don't need a separate `meta.wait` in front purely to guard the write. Add a leading `meta.wait` only if you want a single combined hold on the verdict *before* either write is even prepared for drafting.
- **Code review** — someone else's PR (community, teammate, dependabot-ish) named by a GitHub PR URL in the job title/description. → one `orchestrated` step, `controller: "code.orchestrate-review"`, `inputs.prUrl` set to that URL, `workspace: { "kind": "readonly", "repoCwd": <registered project path>, "ref": "refs/pull/<N>/head" }` (Step 3 covers resolving the path). Set `inputs.until: "closed"` only when the job text says the reviewer stays on the PR through merge (a deployment/release PR); otherwise leave it unset and the action's default (`"approved"`) applies. Add `inputs.goal` only when the review is narrower than a general one.
...
**Code review steps resolve the repo from the PR URL, not the ticket prose.** Parse `owner/repo` out of the `https://github.com/<owner>/<repo>/pull/<n>` URL and match it (case-insensitive) against the registered-projects list above. Matched → use that path as `repoCwd`. Unmatched but resolvable (`gh repo view <owner>/<repo>` succeeds) → prepend a `write.add-project` step ahead of the review step, with `inputs: { "repo": "<owner>/<repo>", "dest": "<parent>/<repo>" }` where `<parent>` is the directory hosting the most registered projects (the same convention `write.add-project` itself falls back to; use `$HOME/<owner>/<repo>` if the registry is empty) — pick the path yourself so the review step's `workspace.repoCwd` can point at that exact `dest`. Genuinely unresolvable (a private repo you have no access to, an `owner/repo` that 404s) → keep the existing fallback: don't emit the step, mention it in your final stdout message.
